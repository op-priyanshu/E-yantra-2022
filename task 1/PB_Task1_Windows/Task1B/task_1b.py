...
def detect_Qr_details(image):
    ...
    for barcode in barcodes:
        ...
        Qr_codes_details[f"{barcodeData}"] = centerlis
...
def detect_ArUco_details(image):
    ...
    for box in bbox:
        ...
        x_coordinate = int(x1+val)
        x2 = abs(box[0][0][1]-box[0][2][1])/2
        val2 = min(box[0][0][1],box[0][2][1])
        y_coordinate = int(x2+val2)
        center_lis.append(x_coordinate)
        center_lis.append(y_coordinate)
        lis.append(center_lis)
        ...
        if(result>0):
            ...
            result = int(-(180+result))
            lis.append(result)
            ArUco_details_dict[int(ids[counter][0])] = lis
            ArUco_corners[int(ids[counter])] = box[0]
            counter = counter+1
